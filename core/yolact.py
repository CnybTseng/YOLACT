...
class Yolact(nn.Module):
    def __init__(self, cfg):
        ...
        if cfg.mask.type == 'linearcombine':
            if cfg.mask.prototype.use_grid:
                ...
                self.num_grids = 0
            self.prototype_source = cfg.mask.prototype.source
            if self.prototype_source is None:
                in_channels = 3
            elif cfg.fpn is not None:
                in_channels = cfg.fpn.num_features
            else:
                in_channels = self.backbone.channels[self.prototype_source]
    ...
    def forward(self, x):
        ...
        if self.fpn is not None:
            ys = [ys[i] for i in self.backbone_selected_layers]
            ys = self.fpn(ys)
        return ys